...
export const voteForAddressList = async function (hre: HardhatRuntimeEnvironment, args: TaskArguments) {
    ...
    if (addressList.length) {
        ...
        console.log(`current hat: ${hat}`)


        // console.log(addressList, chiefContract)
        const result = await submitTx(
            chiefContract["vote(address[])"](addressList)
        );
        console.log(result)
    }
}
