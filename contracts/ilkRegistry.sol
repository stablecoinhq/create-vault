...
contract IlkRegistry {
    ...
    function add(address adapter) external {
        ...
        // Validate adapter
        require(
            _join.vat() == address(vat),
            "IlkRegistry/invalid-join-adapter-vat"
        );
        require(
            vat.wards(address(_join)) == 1,
            "IlkRegistry/adapter-not-authorized"
        );
        ...
        require(
            ilkData[_ilk].join == address(0),
            "IlkRegistry/ilk-already-exists"
        );
    ...
    function file(
        ...
        if (what == "class") {
            require(data <= uint96(-1) && data != 0);
            ilkData[ilk].class = uint96(data);
        } else if (what == "dec") {
            require(data <= uint8(-1));
            ilkData[ilk].dec = uint8(data);
    ...
    function list(uint256 start, uint256 end)
        external
        view
    ...
    function update(bytes32 ilk) external {
        require(
            JoinLike(ilkData[ilk].join).vat() == address(vat),
            "IlkRegistry/invalid-ilk"
        );
        require(
            JoinLike(ilkData[ilk].join).live() == 1,
            "IlkRegistry/ilk-not-live-use-remove-instead"
        );
        uint96 _class = ilkData[ilk].class;
        require(_class == 1 || _class == 2, "IlkRegistry/invalid-class");
    ...
    function put(
        ...
        require(
            _class != 0 && _class <= uint96(-1),
            "IlkRegistry/invalid-class"
        );
