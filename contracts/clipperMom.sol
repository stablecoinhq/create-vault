...
contract ClipperMom {
    ...
    function getPrices(address clip)
        internal
        view
        returns (uint256 cur, uint256 nxt)
    {
    ...
    */
    function tripBreaker(address clip) external {
        require(
            ClipLike(clip).stopped() < 2,
            "ClipperMom/clipper-already-stopped"
        );
        ...
        require(
            nxt < rmul(cur, tolerance[clip]),
            "ClipperMom/price-within-bounds"
        );
